...
export const useLocation = (taskName: string, onReceiveLocations: (locations: Location.LocationGeocodedLocation[]) => void) => {
    // 0. Define new task
    TaskManager.defineTask(taskName, async ({ data: { locations }, error }: LocationTaskArgs) => {
        if (error) {
        ...
        const geocodedAddress: Location.LocationGeocodedAddress[] = await Location.reverseGeocodeAsync(locations[locations.length-1], geocodingOptions);
    ...
    useEffect(() => {
        const requestPermissions = async () => {
            ...
            if(!!foregroundStatus && !foregroundStatus.granted) await requestForegroundPermission();
            if(!!backgroundStatus && !backgroundStatus.granted) await requestBackgroundPermission();

            // 2. Have permissions
            if(!!foregroundStatus && foregroundStatus.granted && !!backgroundStatus && backgroundStatus.granted) {                
