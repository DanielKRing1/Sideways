...
import { Animated, TouchableOpacity, TouchableOpacityProps, useWindowDimensions } from 'react-native';
...
const OutputHeatMapCell: FC<HeatMapCellProps> = (props) => {
    ...
    const margin = useMemo(() => {
        ...
        const bMargin: string = Math.floor(data.index / gridDim.x) < lastRow ? `${width / 100}px` : `0px`;
        const rMargin: string = (data.index + 1) % gridDim.x !== 0 ? `${width / 100}px` : `0px`;
