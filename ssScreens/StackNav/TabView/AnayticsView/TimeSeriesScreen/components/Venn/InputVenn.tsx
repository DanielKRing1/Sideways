...
import { getDecorationMapValue, getDecorationMapSubsetList } from 'ssDatabase/hardware/realm/userJson/utils';
...
const InputVenn: FC<InputVennProps> = (props) => {
    ...
    const { activeSliceName, vennByMonth, vennNodeInputs, monthIndex, fullDecorationMap } = useSelector((state: RootState) => ({ ...state.readSidewaysSlice.toplevelReadReducer, ...state.analyticsSlice.timeseriesStatsSlice, ...state.userJsonSlice.decorationSlice }));
    ...
    const colorScale: string[] = useMemo(() => {        
        ...
        return getDecorationMapSubsetList<string>(DECORATION_ROW_KEY.INPUT, vennNodeIds, DECORATION_VALUE_KEY.COLOR, fullDecorationMap, (i: number, value: string) => value);
    ...
    return (
        <View>
            ...
            <VennStackWSlider
                ...
                xLabelFill={({ text }) => getDecorationMapValue(DECORATION_ROW_KEY.OUTPUT, text[0], fullDecorationMap)[DECORATION_VALUE_KEY.COLOR]}
