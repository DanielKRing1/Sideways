...
const Slider: FC<SliderProps> = props => {
  const {
    ...
    isPressed,
    offsetX,
    delta = useSharedValue(Infinity),
    onChangeValue,
    onChangeDelta = undefined,
    minValue,
    maxValue,
    step,
  } = props;
  ...
  const AnimatedComponent = React.useMemo(
    () => createSliderButton(isPressed, offsetX),
    [createSliderButton],
  );
  ...
  const gesture = Gesture.Pan()
    ...
    .onUpdate(e => {
      ...
      if (newX <= right.value && newX >= left.value) {
        ...
        const curValue: number =
          offsetX.value === 0
            ? minValue
            : (Math.floor(offsetX.value / stepUnit.value) + minValue) / step;
        ...
        if (
          onChangeDelta !== undefined &&
          Math.abs(offsetX.value - lastX.value) >= delta.value
        ) {
