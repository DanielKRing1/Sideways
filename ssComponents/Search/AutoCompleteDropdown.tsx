...
import { SearchableDropdown, SearchableDropdownProps } from 'ssComponents/Search/SearchableDropdown';
...
const AutoCompleteDropdown: FC<AutoCompleteDropdownProps<any>> = (props) => {
    const {
        ...
        allSuggestions: _allSuggestions, getSuggestionText=(suggestion: string) => suggestion,
        DropdownRow
    } = props;

    const [ allSuggestions, setAllSuggestions ] = useState<string[]>([]);

    const { setValues: setTrieValues, search, autoComplete } = useTrie<any>(getSuggestionText);
    ...
    useEffect(() => { search(inputValue) }, [inputValue]);
    ...
    const Dropdown: FC<{}> = () => (
        <FlexCol>
        {
            ...
            ?
            autoComplete.map((suggestion: any) => <DropdownRow suggestion={suggestion} />)
            :
            // No suggestions
