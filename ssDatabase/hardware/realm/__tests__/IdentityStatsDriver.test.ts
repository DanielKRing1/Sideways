...
const TEST_GROWING_ID_TEXT_ALL: NewSliceOutput[] = TEST_OUPUTS_ALL.map((output: string, i) => ({ id: i, text: output }));
        ...
        for(const monthSnapshot of modifiedActual) {
            for(let i = 0; i < monthSnapshot.venn.length; i++) {
                const nodeSnapshots: ChartBar[] = monthSnapshot.venn[i];
                 monthSnapshot.venn[i]= nodeSnapshots.map<ChartBar>((daySnapshot: ChartBar) => ({ ...daySnapshot, x: new Date(0) }));
            }
        }

        expect(modifiedActual)
            .toEqual<VennByMonth[]>([
                {
                    timestamp: new Date(0),
                    venn: [
                        [ {x: new Date(0), y: 1, y0: 0}, {x: new Date(0), y: 1, y0: 0} ],
                        [ {x: new Date(0), y: 1, y0: 1}, {x: new Date(0), y: 1, y0: 1} ],
                        [ {x: new Date(0), y: 1, y0: 2}, {x: new Date(0), y: 1, y0: 2} ],
                    ],
                    outputs: [ TEST_RATE_4.outputs, TEST_RATE_3.outputs, TEST_RATE_2.outputs, TEST_RATE_1.outputs ]
                }
            ]);
    });
