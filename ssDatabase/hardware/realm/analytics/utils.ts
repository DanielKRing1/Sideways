...
export const rankedNodesMapToRankedNodes = (rankedNodesMap: RankedNodesMap): RankedNode[] => Object.keys(rankedNodesMap).map((nodeId: string) => ({ ...rankedNodesMap[nodeId], id: nodeId }));
 ...
 * @returns 
 */
 export const sortRankedNodesByOutput = (rankedNodes: RankedNode[], targetOutput: string, outputType?: OutputKeyType): RankedNode[] => {
    const targetOutputKey: string = getOutputKey(targetOutput, outputType);
    
    return rankedNodes.sort((a: RankedNode, b: RankedNode) => a[targetOutputKey]-b[targetOutputKey]);
}
 ...
 * @returns 
 */
 export const sortRankedNodesMapByOutput = (rankedNodesMap: RankedNodesMap, targetOutput: string, outputType?: OutputKeyType): RankedNode[] => {
 ...
 * @returns 
 */
export const filterCGEntityAttrs = (cgentity: CGNode | CGEdge, outputs: string[], outputType?: OutputKeyType): RankedNode => {
 ...
 * @returns 
 */
export const getDestinationNodeId = (cgedge: CGEdge, originNodeId: string): string => cgedge.nodeId1 !== originNodeId ? cgedge.nodeId1 : cgedge.nodeId2;
 ...
 * @returns 
 */
 export const getOutputKeys = (outputs: string[], outputType?: OutputKeyType): string[] => (
